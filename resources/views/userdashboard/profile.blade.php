...
@section('dashboard_content')
    <div
        style="background:#fff; border-radius:10px; padding:30px; box-shadow:0 2px 6px rgba(0,0,0,0.1); max-width:700px; margin:auto;">
        <h2 style="color:#28a745; margin-bottom:20px;">👤 My Profile</h2>

        <form action="{{ route('profile.update') }}" method="POST" style="display:grid; grid-template-columns:1fr 1fr; gap:20px;">
            @csrf
            ...
            <div style="display:flex; flex-direction:column;">
                ...
                <input type="text" name="phone" value="{{ $latestOrder->billing_phone_number ?? '' }}"
                    style="padding:10px; border:1px solid #ccc; border-radius:6px;">
            </div>

            <div style="display:flex; flex-direction:column;">
                <label style="margin-bottom:6px; color:#155d27;">Address</label>
                <input type="text" name="address" value="{{ $latestOrder->billing_street_address ?? '' }}"
                    style="padding:10px; border:1px solid #ccc; border-radius:6px;">
            </div>
