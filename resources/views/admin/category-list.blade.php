...
<style>
    ...
    object-fit: cover; /* image crop ho ke fit ho jayegi */
...
<div class="container">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2>Category List</h2>
        <a href="#" class="btn  mb-3"  style="background-color: black; border-color: black; color: white;" id="openAddCategoryModal">+ Add Category</a>
    </div>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                 @if($cat->image)
        <img src="{{ asset('images/' . $cat->image) }}" width="80" height="80" alt="Category Image">
    @else
    ...
    @endif
     ...
     {{-- data-description="{{ $cat->description }}" --}}
                            data-available="{{ $cat->available_items }}"
                            ...
                            <li>
                                <form action="{{ route('admin.category.delete', $cat->id) }}" method="POST" class="deleteCategoryForm">
                                    @csrf
                                    @method('DELETE')
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="categoryForm" action="{{ route('admin.category.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <input type="text" id="name" name="name" placeholder="Enter category name" required class="form-control">
            ...
            <input type="number" id="available_items" name="available_items" min="0" placeholder="Enter available items" class="form-control" required>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="updateCategoryForm" action="#" method="POST"  enctype="multipart/form-data">
          ...
          <textarea id="update_description" name="description" class="form-control"></textarea> --}}

          <label for="update_available_items">Available Items</label>
          <input type="number" id="update_available_items" name="available_items" min="0" class="form-control" required>
          ...
          <input type="file" id="update_image" name="image" accept="image/*" class="form-control">
          <div id="update_imagePreview" style="margin-top:10px;">No image selected</div>
...
{{-- VIEW DETAILS PAGE --}}
<!-- View Category Modal -->
<div class="modal fade" id="viewCategoryModal" tabindex="-1" aria-labelledby="viewCategoryLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
        ...
        <div class="card text-center">
          <div class="card-body">
            <div id="view_imagePreview" style="margin-bottom:15px;">
              <img src="" alt="Category Image" class="img-fluid rounded" style="max-height:150px;">
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.editCategoryBtn').forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            if(this.dataset.image){
                imgPreview.innerHTML = `<img src="/images/${this.dataset.image}" alt="Category Image">`;
            } else {
                imgPreview.innerHTML = 'No image selected';
            }
    ...
    updateImageInput.addEventListener('change', function() {
        ...
        if(file){
            const reader = new FileReader();
            reader.onload = function(e){
                updateImagePreview.innerHTML = `<img src="${e.target.result}" alt="Category Image">`;
            }
            reader.readAsDataURL(file);
        } else {
            updateImagePreview.innerHTML = 'No image selected';
        }
    });
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.viewCategoryBtn').forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            document.getElementById('view_available_items').innerText = this.dataset.available;
            ...
            const imgPreview = document.getElementById('view_imagePreview').querySelector("img");
            if (this.dataset.image) {
