...
<div class="d-flex justify-content-center align-items-center" style="min-height: 80vh;">
    <form class="category-form" action="{{ route('admin.category.update', $category->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <h2>Add New Category</h2>

        <label for="name">Category Name</label>
        <input type="text" id="name" value="{{$category->name}}" name="name" placeholder="Enter category name" required>

        {{-- <label for="slug">Slug (URL Friendly)</label>
        <input type="text" id="slug" name="slug" placeholder="e.g., fast-food" required> --}}

        <label for="description">Description</label>
        <textarea id="description" name="description"  placeholder="Enter category description">{{$category->description}}</textarea>
